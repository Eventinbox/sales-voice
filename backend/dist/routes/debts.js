...
router.get("/", async (req, res) => {
    ...
    const debts = await DebtRepository_1.debtRepository.findAll(type);
    res.json(debts);
});
// GET /api/debts/:id
router.get("/:id", async (req, res) => {
    const debt = await DebtRepository_1.debtRepository.findById(req.params.id);
    if (!debt)
...
router.patch("/:id", async (req, res) => {
    ...
    try {
        const debt = await DebtRepository_1.debtRepository.updateStatus(req.params.id, status);
        res.json(debt);
    }
    catch {

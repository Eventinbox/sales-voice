...
import { DebtEntry, PriceBenchmark } from "@/lib/types";
import { fetchDebt, fetchPrices, updateDebtStatus } from "@/lib/api";
...
export default function DebtDetailPage({ params }: { params: { id: string } }) {
  ...
  const benchmark = prices.find((price) => price.item.toLowerCase() === debt.item.toLowerCase());
    ...
    <div className="px-5 py-6 space-y-8">
        ...
        <div className="bg-surface-container-lowest border border-surface-container-high rounded-market p-4 space-y-4">
          <ListRow
            icon={
              <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24">
                <path d="M7 18c-1.1 0-1.99.9-1.99 2S5.9 22 7 22s2-.9 2-2-.9-2-2-2zM1 2v2h2l3.6 7.59-1.35 2.45a.5.5 0 00.55.65h11.5a.5.5 0 00.55-.65l-1.35-2.45 3.6-7.59H21V2H1z" />
              </svg>
            }
            label="Item"
            value={debt.item}
          ...
          <div className="h-[64px] flex items-center justify-between px-2 gap-4">
            <div className="flex items-center gap-3">
              ...
              >
                <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M11.99 2C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zM12 20c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8zm.5-13H11.5v6l4.25 2.5c.41.25.5.7.25 1.1h-2.5c-.25.41-.7.5-1.1-.25L12 12.5v-6z" />
                </svg>
              </div>
      ...
      <section className="space-y-4">
        <div className="flex justify-between items-center">
          ...
          {benchmark && <StatusPill label="Market Benchmark" variant="outline" color="primary" />}
        </div>
        {benchmark ? (
          <PriceRangeBar current={benchmark.currentPrice} min={benchmark.minMarketPrice} max={benchmark.maxMarketPrice} />
        ) : (
          <p className="text-body-md text-on-surface-variant">
            No market benchmark is available for this item yet.
          </p>
        )}
      </section>
